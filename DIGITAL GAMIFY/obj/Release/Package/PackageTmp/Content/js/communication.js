...
function SetCustomersPage() {
    ...
    $('#btnShow').on("click", function (event) {
        ...
        if (hasErrors == false) {
            window.location.href = GetUrlWithNoQueryStrings() + "?_f=" + $("#from").val() + "&_t=" + $("#to").val() + "&_pi=" + pi + "&_str=" + $("#search").val() + "&_age=" + $("#Agegroup").val() + "&_g=" + $("#gender").val();
        }
    ...
    function SetViewMembersGrid(pageindex, pagesize) {
        ...
        gl.ajaxreqloader(websiteurlapi + "/GetGuestCheckInByBusiness", "POST", { BusinessId: bid, Pi: pageindex, Ps: pagesize, age: age, str: str, gender: gender }, function (response) {
            if (response.length > 0) {
                ...
                $.each(response, function (i, item) {
                    ...
                    if (indx != -1) {
                        row += "<tr><td><input class='chkbox' id='chkid_" + item.CustomerId + "' checked='checked'  type='checkbox'></td><td class='col-lg-2'>" + item.FirstName + ' ' + item.LastName + "</td><td class='col-lg-4'>" + item.Mobile + "</td><td class='col-lg-2'>" + item.Email + "</td><td class='col-lg-2'>" + item.Agestring + "</td><td class='col-lg-2'>" + item.Genderstring + "</td><td class='col-lg-2'>" + item.ZipCode + "</td></tr>";
                    }
                    else {
                        row += "<tr><td><input class='chkbox' id='chkid_" + item.CustomerId + "' type='checkbox'><td class='col-lg-2'>" + item.FirstName + ' ' + item.LastName + "</td><td class='col-lg-4'>" + item.Mobile + "</td><td class='col-lg-2'>" + item.Email + "</td><td class='col-lg-2'>" + item.Agestring + "</td><td class='col-lg-2'>" + item.Genderstring + "</td><td class='col-lg-2'>" + item.ZipCode + "</td></tr>";
    ...
    $(document).on("click", ".d-paging", function (event) {
        ...
        window.history.pushState(null, "", GetUrlWithNoQueryStrings() + "?_f=" + $("#from").val() + "&_t=" + $("#to").val() + "&_pi=" + pageindex + "&_ps=" + pagesize + "&_str=" + $("#search").val() + "&_age=" + $("#Agegroup").val() + "&_g=" + $("#gender").val());
